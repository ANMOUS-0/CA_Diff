...
def center_crop(data, crop_size, seg=None, aux=None, aux_type:str=None):
    return crop(data, seg, aux, crop_size, 0, 'center', aux_type=aux_type)
...
def crop(data, seg=None, aux=None, crop_size=128, margins=(0, 0, 0), crop_type="center",
    ...
    if aux is not None:
        ...
        aux_shape = OrderedDict()
        aux_dtype = OrderedDict()
        for k,v in aux.items():
            aux_shape[k] = tuple([len(v)] + list(v[0].shape))
            aux_dtype[k] = v[0].dtype

            if not isinstance(v, (list, tuple, np.ndarray)):
                raise TypeError("data has to be either a numpy array or a list")
            
            assert all([i == j for i, j in zip(aux_shape[k][2:], data_shape[2:])]), "data and aux must have the same spatial " \
                                                                                "dimensions. Data: %s, aux: %s" % \
                                                                                (str(data_shape), str(aux_shape[k]))
    ...
    if aux is not None:
        aux_return = OrderedDict()
        for k,v in aux_shape.items():
            aux_return[k] = np.zeros([v[0], v[1]] + list(crop_size), dtype=aux_dtype[k])
    else:
        aux_return = None
    ...
    for b in range(data_shape[0]):
        ...
        if aux is not None:
            aux_shape_here = OrderedDict()
            for k,v in aux.items():
                aux_shape_here[k] = [aux_shape[k][0]] + list(v[b].shape)
        ...
        if aux_return is not None:
            aux_cropped = OrderedDict()
            for k,v in aux.items():
                slicer_aux = [slice(0, aux_shape_here[k][1])] + [slice(lbs[d], ubs[d]) for d in range(dim)]
                aux_cropped[k] = v[b][tuple(slicer_aux)]      
        ...
        if any([i > 0 for j in need_to_pad for i in j]):
            ...
            if aux_return is not None:
                for i,(k,v) in enumerate(aux_cropped.items()):
                    aux_return[k][b] = np.pad(v, need_to_pad, 
                                            pad_mode if aux_type[i]=='data' else pad_mode_seg,
                                            **(pad_kwargs if aux_type[i]=='data' else pad_kwargs_seg)
                                            )
        else:
            ...
            if aux_return is not None:
                for k,v in aux_cropped.items():
                    aux_return[k][b] = aux_cropped[k]

    return data_return, seg_return, aux_return


def random_crop(data, seg=None, aux=None, crop_size=128, margins=[0, 0, 0], aux_type:str=None):
    return crop(data, seg, aux, crop_size, margins, 'random', aux_type=aux_type)
