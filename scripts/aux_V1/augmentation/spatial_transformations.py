...
# WITHOUT WARRANTIES OR auxS OF ANY KIND, either express or implied.
...
def augment_mirroring(sample_data, sample_seg=None, sample_aux=None, axes=(0, 1, 2)):
    ...
    if 2 in axes and len(sample_data.shape) == 4:
        if np.random.uniform() < 0.5:
            ...
            if sample_aux is not None:
                sample_aux[:, :, :, :] = sample_aux[:, :, :, ::-1]
    return sample_data, sample_seg, sample_aux
...
def augment_spatial(data, seg, aux, patch_size, patch_center_dist_from_border=30,
                    ...
                    border_cval_aux:List[str]=None,
                    order_aux:List[int]=None):
    dim = len(patch_size)
    seg_result = None
    if seg is not None:
        if dim == 2:
    ...
    if aux is not None:
        ...
        "Wrong in spatial augmentation: This version aux package require aux inputs stored in a dict."
        aux_result = OrderedDict()
        for aux_indicator, aux_value in aux.items():
            if dim==2:
                aux_result[aux_indicator] = \
                np.zeros((aux_value.shape[0], aux_value.shape[1], patch_size[0], patch_size[1]), dtype=np.float32)
            else:
                aux_result[aux_indicator] = \
                np.zeros((aux_value.shape[0], aux_value.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                         dtype=np.float32)
    ...
    for sample_id in range(data.shape[0]):
        ...
        if modified_coords:
            ...
            # resample each aux inputs;        
            for i, (aux_indicator,aux_value) in enumerate(aux.items()):
                if aux_type[i]=='data':
                    for channel_id in range(aux_value.shape[1]):
                        aux_result[aux_indicator][sample_id, channel_id] = \
                        interpolate_img(aux_value[sample_id, channel_id], coords, 
                                        order_aux[i] if order_aux[i] is not None else order_data,
                                        border_mode_data, 
                                        cval=border_cval_aux[i] if border_cval_aux[i] is not None else border_cval_data)
                else:
                    for channel_id in range(aux_value.shape[1]):
                        aux_result[aux_indicator][sample_id, channel_id] = \
                        interpolate_img(aux_value[sample_id, channel_id], coords, 
                                        order_aux[i] if order_aux[i] is not None else order_seg,
                                        border_mode_seg, 
                                        cval=border_cval_aux[i] if border_cval_aux[i] is not None else border_cval_seg,
                                        is_seg=True)
        else:
            if seg is None:
                s = None
            ...
            c = OrderedDict()
            for aux_indicator, aux_value in aux.items():
                c[aux_indicator] = aux_value[sample_id:sample_id + 1]

            if random_crop:
                margin = [patch_center_dist_from_border[d] - patch_size[d] // 2 for d in range(dim)]
                d, s, c = random_crop_aug(data[sample_id:sample_id + 1], s, c, patch_size, margin, aux_type=aux_type)
            else:
                d, s, c = center_crop_aug(data[sample_id:sample_id + 1], patch_size, s, c, aux_type=aux_type)
            data_result[sample_id] = d[0]
            if seg is not None:
                seg_result[sample_id] = s[0]
            if aux is not None:
                for k,v in c.items():
                    aux_result[k][sample_id] = v[0]
    return data_result, seg_result, aux_result
...
def augment_spatial_intra_batch_same(data, seg, aux, patch_size, patch_center_dist_from_border=30,
                    do_elastic_deform=True, alpha=(0., 1000.), sigma=(10., 13.),
                    do_rotation=True, angle_x=(0, 2 * np.pi), angle_y=(0, 2 * np.pi), angle_z=(0, 2 * np.pi),
                    do_scale=True, scale=(0.75, 1.25), border_mode_data='nearest', border_cval_data=0, order_data=3,
                    border_mode_seg='constant', border_cval_seg=0, order_seg=0, random_crop=True, p_el_per_sample=1,
...
def augment_spatial_intra_batch_same_v1(data, seg, aux, patch_size, patch_center_dist_from_border=30,
                    do_elastic_deform=True, alpha=(0., 1000.), sigma=(10., 13.),
                    do_rotation=True, angle_x=(0, 2 * np.pi), angle_y=(0, 2 * np.pi), angle_z=(0, 2 * np.pi),
                    do_scale=True, scale=(0.75, 1.25), border_mode_data='nearest', border_cval_data=0, order_data=3,
                    border_mode_seg='constant', border_cval_seg=0, order_seg=0, random_crop=True, p_el_per_sample=1,
    ...
    broadcasted_array = random_tensor.cpu().numpy()
    ...
    for sample_id in range(data.shape[0]):
        ...
        if modified_coords:
            ...
            if aux is not None:
                for k,v in c.items():
                    aux_result[k][sample_id] = v[0]
    return data_result, seg_result, aux_result, broadcasted_array
