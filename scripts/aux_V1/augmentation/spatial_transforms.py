...
from .spatial_transformations import augment_spatial_2, \
    augment_channel_translation, \
    augment_mirroring, augment_transpose_axes, augment_zoom, augment_resize, augment_rot90
import numpy as np
from .spatial_transformations import augment_spatial, augment_spatial_intra_batch_same, augment_spatial_intra_batch_same_v1
...
class MirrorTransform(AbstractTransform):
    ...
    def __init__(self, axes=(0, 1, 2), data_key="data", label_key="seg", aux_key="aux", p_per_sample=1):
        self.p_per_sample = p_per_sample
        self.data_key = data_key
        self.label_key = label_key
    ...
    def __call__(self, **data_dict):
        data = data_dict.get(self.data_key)
        seg = data_dict.get(self.label_key)
        aux = data_dict.get(self.aux_key)
        ...
        for b in range(len(data)):
            if np.random.uniform() < self.p_per_sample:
                ...
                if aux is not None:
                    sample_aux = aux[b]
                ret_val = augment_mirroring(data[b], sample_seg, sample_aux, axes=self.axes)
                data[b] = ret_val[0]
                if seg is not None:
                    seg[b] = ret_val[1]
                if aux is not None:
...
class SpatialTransform(AbstractTransform):
    ...
    def __init__(self, patch_size, patch_center_dist_from_border=30,
        ...
        self.border_cval_aux = border_cval_aux
        self.order_aux = order_aux
    ...
    def __call__(self, **data_dict):
        ...
        ret_val = augment_spatial(data, seg, aux, patch_size=patch_size,
                                  ...
                                  aux_type=self.aux_type,border_cval_aux=self.border_cval_aux,
                                  order_aux=self.order_aux)
        data_dict[self.data_key] = ret_val[0]
        if seg is not None:
            data_dict[self.label_key] = ret_val[1]
        if aux is not None:
            data_dict[self.aux_key] = ret_val[2]
...
class SpatialTransformIntraBatchSame(AbstractTransform):
    """The ultimate spatial transform generator. Rotation, deformation, scaling, cropping: It has all you ever dreamed
    of. Computational time scales only with patch_size, not with input patch size or type of augmentations used.
    Internally, this transform will use a coordinate grid of shape patch_size to which the transformations are
    applied (very fast). Interpolation on the image data will only be done at the very end
    ...
    def __call__(self, **data_dict):
        ...
        ret_val = augment_spatial_intra_batch_same(data, seg, aux, patch_size=patch_size,
                                  patch_center_dist_from_border=self.patch_center_dist_from_border,
                                  do_elastic_deform=self.do_elastic_deform, alpha=self.alpha, sigma=self.sigma,
                                  do_rotation=self.do_rotation, angle_x=self.angle_x, angle_y=self.angle_y,
                                  angle_z=self.angle_z, do_scale=self.do_scale, scale=self.scale,
...
class SpatialTransformIntraBatchSameV1(AbstractTransform):
    """The ultimate spatial transform generator. Rotation, deformation, scaling, cropping: It has all you ever dreamed
    of. Computational time scales only with patch_size, not with input patch size or type of augmentations used.
    Internally, this transform will use a coordinate grid of shape patch_size to which the transformations are
    applied (very fast). Interpolation on the image data will only be done at the very end
    ...
    def __call__(self, **data_dict):
        ...
        ret_val = augment_spatial_intra_batch_same_v1(data, seg, aux, patch_size=patch_size,
                                  patch_center_dist_from_border=self.patch_center_dist_from_border,
                                  do_elastic_deform=self.do_elastic_deform, alpha=self.alpha, sigma=self.sigma,
                                  do_rotation=self.do_rotation, angle_x=self.angle_x, angle_y=self.angle_y,
                                  angle_z=self.angle_z, do_scale=self.do_scale, scale=self.scale,
        ...
        if aux is not None:
            data_dict[self.aux_key] = ret_val[2]
        if len(ret_val)==4:
            data_dict['broadcast_array'] = ret_val[3]
