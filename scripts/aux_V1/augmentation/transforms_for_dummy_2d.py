...
class Convert3DTo2DTransform(AbstractTransform):
    def __init__(self, apply_to_keys: Union[List[str], Tuple[str]] = ('data', 'seg')):
        ...
        Support dict inputs like multi aux inputs: {'aux1':... , 'aux2':...}.
        """
    ...
    def __call__(self, **data_dict):
        for k in self.apply_to_keys:
            if isinstance(data_dict[k], dict):
                for key,_ in data_dict[k].items():
                    shp = data_dict[k][key].shape
                    assert len(shp) == 5, 'This transform only works on 3D data, so expects 5D tensor (b, c, x, y, z) as input.'
                    data_dict[k][key] = data_dict[k][key].reshape((shp[0], shp[1] * shp[2], shp[3], shp[4]))
                    shape_key = f'orig_shape_{k}_{key}'
...
class Convert2DTo3DTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        for k in self.apply_to_keys:
            if isinstance(data_dict[k], dict):
                for key,_ in data_dict[k].items():
                    if not key.startswith('orig_shape_'):   # skip shape items when reshape inputs;
                        shape_key = f'orig_shape_{k}_{key}'
                        ...
                        current_shape = data_dict[k][key].shape
                        data_dict[k][key] = data_dict[k][key].reshape(
