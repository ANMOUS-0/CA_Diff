...
class nnUNetDataLoaderBase(DataLoader):
    def __init__(self,
                 ...
                 aux_border_cval:float=0):
        '''
        aux_border_cval: the value used to pad aux input border value;
        '''
        ...
        self.data_shape, self.seg_shape, self.aux_shape = self.determine_shapes()
    ...
    def determine_shapes(self):
        # load one case
        data, seg, aux, properties = self._data.load_case(self.indices[0])
        ...
        assert isinstance(aux, dict), \
            "In this version, the aux inputs should be a dict incase of more than one aux is used."
        aux_shape = OrderedDict()
        for aux_indicator,aux_value in aux.items():
            aux_shape.update(
                {aux_indicator : (self.batch_size, aux_value.shape[0], *self.patch_size)}
                )

        return data_shape, seg_shape, aux_shape
