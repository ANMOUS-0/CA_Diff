...
class nnUNetDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
                 ...
                 aux_indicator:List[str]=None, 
                 aux_type:List[str]=None):
        """
        ...
        Custom modified to support special inputs besides image and label.
        """
        super().__init__()
        ...
        for c in case_identifiers:
            ...
            # Support multi aux inputs;
            for idx in range(len(aux_indicator)):
                assert self.aux_type[idx] in ['data', 'seg'], f'aux type not supported: {self.aux_type}'
                self.dataset[c][f'aux_{aux_indicator[idx]}_file'] = join(folder, f'{c}_aux_{aux_indicator[idx]}.npy')
    ...
    def load_case(self, key):
        ...
        # try to build a dict for multi aux inputs;
        aux = OrderedDict()
        for i in range(len(self.aux_indicator)):
            aux.update(
                {self.aux_indicator[i] : np.load(entry[f'aux_{self.aux_indicator[i]}_file'], 'r')}
                )
            if self.aux_indicator[i]=='seg':
                contains_neg_one = np.any(self.aux_indicator[i] == -1)
        ...
        return data, seg, aux, entry['properties']
