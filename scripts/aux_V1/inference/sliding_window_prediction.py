...
def maybe_mirror_and_predict(network: nn.Module, x: torch.Tensor, 
                             model_kwargs=None, mirror_axes: Tuple[int, ...] = None) \
        -> torch.Tensor:
    prediction = network(image=x, model_kwargs=model_kwargs)
...
def predict_sliding_window_return_logits(network: nn.Module,
                                         ...
                                         verbose: bool = True,
                                         seg: Union[np.ndarray, torch.Tensor]=None,
                                         device: torch.device = torch.device('cuda'),
                                         model_kwargs=None,) -> Union[np.ndarray, torch.Tensor]:
    ...
    with torch.no_grad():
        ...
        with torch.autocast(device.type, enabled=True) if device.type == 'cuda' else dummy_context():
            ...
            for k,v in model_kwargs.items():
                if v.shape[1:] == input_image.shape[1:]:
                    image_like_keys.append(k)
                    warnings.simplefilter("ignore")
                    v = torch.from_numpy(v)
                    model_kwargs[k], _ = pad_nd_image(v, tile_size, 'constant', {'value': 0}, True, None)
            ...
            for sl in slicers:
                workon = data[sl][None]
                workon_model_kwargs = deepcopy(model_kwargs)
                for k in image_like_keys:
                    workon_model_kwargs[k] = model_kwargs[k][sl][None].to(device, non_blocking=False)

                workon = workon.to(device, non_blocking=False)

                prediction = maybe_mirror_and_predict(network, workon, workon_model_kwargs, mirror_axes)[0].to(results_device)
