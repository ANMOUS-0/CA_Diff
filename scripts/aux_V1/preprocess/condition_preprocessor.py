...
class AuxiliaryPreprocessor(object):
    '''
    Used to preprocess auxiliary input for auxiliary generation model.
    '''
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], 
                     aux: Union[np.ndarray, None],
                     properties: dict,
                     plans_manager: PlansManager, configuration_manager: ConfigurationManager,
                     dataset_json: Union[dict, str], aux_resample_type:str):
        ...
        if aux is not None:
            aux = aux.transpose([0, *[i + 1 for i in plans_manager.transpose_forward]])
        ...
        data, seg, aux, bbox = crop_to_nonzero(data, seg, aux)
        ...
        if aux_resample_type=='data':
            aux = configuration_manager.resampling_fn_data(aux, new_shape, original_spacing, target_spacing)
        elif aux_resample_type == 'seg':
            aux = configuration_manager.resampling_fn_seg(aux, new_shape, original_spacing, target_spacing)
        else:
            NotImplementedError(f"Wrong aux type: {aux_resample_type}")
        if self.verbose:
        ...
        if aux_resample_type == 'seg':
            unique_aux_label = np.unique(aux)
            if len(unique_aux_label) > 127:
    ...
    def run_case(self, image_files: List[str], seg_file: Union[str, None],
                 aux_file: Union[str, None], 
                 plans_manager: PlansManager,
                 configuration_manager: ConfigurationManager,
                 dataset_json: Union[dict, str],
                 aux_resample_type:str,
                 aux_indicator:str):
        """
        ...
        if aux_file is not None:
            aux, _ = rw.read_seg(aux_file)
        else:
            aux = None

        # print(np.array_equal(seg, aux))

        data, seg, aux = self.run_case_npy(data, seg, aux,
                                      data_properites, plans_manager, configuration_manager,
                                      dataset_json,
                                      aux_resample_type=aux_resample_type)
        return data, seg, aux, data_properites

    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str, 
                      aux_file:str, 
                      plans_manager: PlansManager, configuration_manager: ConfigurationManager,
                      dataset_json: Union[dict, str],
                      aux_resample_type:str, aux_indicator:str):
        data, seg, aux, properties = self.run_case(image_files, seg_file, aux_file, plans_manager, configuration_manager, dataset_json,
                                              aux_resample_type=aux_resample_type, aux_indicator=aux_indicator)
        # print('dtypes', data.dtype, seg.dtype)
        # prevent from duplicate saving or overwritting of image and seg files;
        # np.savez_compressed(output_filename_truncated + '.npz', data=data, seg=seg)   
        write_pickle(properties, output_filename_truncated + '.pkl')
        np.save(output_filename_truncated + f'_aux_{aux_indicator}.npy', aux)
    ...
    def run(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
            num_processes: int, 
            aux_indicator:str, aux_resample_type:str):
        """
        Args:
            aux_indicator str: used to distinguish diffent aux.
        ...
        # return unique image id, originaly obtain from image name and remove channel indicator and file suffix;
        ...
        # ############################
        # collect aux file names
        # ############################

        # assuem the aux files will end with _aux_{aux_name}.nii.gz;
        aux_fnames = [join(nnUNet_raw, dataset_name, 'condsTr', aux_indicator, f'{i}_aux_{aux_indicator}.nii.gz') for i in identifiers]

        assert aux_resample_type in ['data', 'seg'], f"Not implemented sampling method for aux data: {aux_resample_type}"
        _ = ptqdm(self.run_case_save, (output_filenames_truncated, image_fnames, seg_fnames, aux_fnames),
                  processes=num_processes, zipped=True, plans_manager=plans_manager,
                  configuration_manager=configuration_manager,
                  dataset_json=dataset_json, disable=self.verbose,
                  aux_resample_type=aux_resample_type,
                  aux_indicator=aux_indicator)
...
def example_test_case_preprocessing():
    ...
    # specified by plans)
    plans_manager = PlansManager(plans_file)
    _ = pp.run(dataset_name_or_id='503', 
                                 configuration_name='3d_fullres_patch96',
                                 plans_identifier='nnUNetPlans',
                                 num_processes=20,
                                 aux_indicator='GtSeg',
                                 aux_resample_type='seg',
                                 )
