...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    #     ignore_mask = np.zeros_like(seg_ref).astype(int)
    #     for i in ignore_label:
    #         ignore_mask = ignore_mask | (seg_ref==i)
    # else:
...
def compute_metrics_on_folder(folder_ref: str, folder_pred: str, output_file: str,
    ...
    # if contain background in final evaluation
    if 0 in regions_or_labels:
        all_class_mean = {}
        for m in metric_list:
            values = []
            for k in means.keys():
                values.append(means[k][m])
            all_class_mean[m] = np.mean(values)
    ...
    if 0 in regions_or_labels:
        recursive_fix_for_json_export(all_class_mean)
        result = {'metric_per_case': results, 'mean': means, 'foreground_mean': foreground_mean, 'all_class_mean':all_class_mean}
    else:
